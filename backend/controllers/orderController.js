  ...
  try {
    const orders = await prisma.order.findMany({
      include: {
        ...
        // اختياري: إذا أردت جلب تفاصيل المنتجات داخل الطلب
       orderItems: { // إضافة هذا الجزء لجلب المنتجات
          include: {
    ...
    // ملاحظة: حقول phone و address ستظهر تلقائياً لأنها جزء من موديل Order
    res.json(orders);
  } catch (error) {
...
export const updateOrder = async (req, res) => {
  const { id } = req.params; // الحصول على ID الطلب من المسار /:id
  const { status, paymentStatus } = req.body; // القيم القادمة من الواجهة الأمامية
  ...
  try {
    const updatedOrder = await prisma.order.update({
      ...
      data: { 
        status, 
        paymentStatus // تم تغيير الاسم هنا ليتطابق مع قاعدة البيانات
      },
    });
