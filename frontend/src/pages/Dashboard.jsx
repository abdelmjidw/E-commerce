...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col md:flex-row relative">
      ...
      {/* Navbar Mobile - تظهر فقط في الشاشات الصغيرة */}
      ...
      {menuOuvert && (
        <div className="fixed inset-0 bg-black/20 z-40 md:hidden" onClick={() => setMenuOuvert(false)} />
      )}
      ...
      <main className="flex-1 p-4 md:p-8 w-full max-w-full overflow-x-hidden md:ml-64">
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
          <div>
            <h1 className="text-2xl md:text-3xl font-extrabold text-slate-800 tracking-tight">Tableau de bord</h1>
            <p className="text-slate-500 mt-1 font-medium italic">Gestion en temps réel.</p>
          </div>
          ...
          {statsCards.map((card, idx) => (
            <div key={idx} className="bg-white border border-slate-200 p-6 rounded-3xl shadow-sm hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="bg-slate-50 p-1 rounded-lg"><ArrowUpRight className="text-slate-300" size={16} /></div>
              </div>
              <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">{card.title}</p>
              <h3 className="text-xl md:text-2xl font-black mt-1 text-slate-800">{card.value}</h3>
            </div>
          ))}
        </div>
            ...
            <div className="h-64 md:h-80 w-full min-w-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data?.topProducts || []}>
                  ...
                  <Bar dataKey="totalSold" fill="#2563eb" radius={[6, 6, 0, 0]} barSize={35} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
              ...
              {data?.recentOrders?.length > 0 ? (
                data.recentOrders.map((order) => (
                  <div key={order.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl border border-slate-100">
                    <div>
                      <p className="font-bold text-slate-700 text-sm truncate max-w-[120px]">{order.user.name}</p>
                      <p className="text-xs font-bold text-blue-600">{order.totalPrice} DH</p>
                    </div>
                    <span className={`text-[9px] px-2 py-1 rounded-full font-black ${order.status === "PAID" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
                      {order.status}
                    </span>
                  </div>
                ))
              ) : (
                <div className="text-center py-10 opacity-40"><p>Aucune commande.</p></div>
              )}
            </div>
          </div>
        </div>
